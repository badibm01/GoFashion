...
class pos_call_log(models.Model):
    ...
    call_results = fields.Binary('Results', attachment=False, readonly=1)
    call_model = fields.Char('Model', required=1, index=True, readonly=1)
    call_fields = fields.Char('Fields', index=True, readonly=1)
    active = fields.Boolean('Active', default=True, readonly=1)
    ...
    def refresh_logs(self, manual=False):
...
class PosQueryLog(models.Model):
    ...
    def clearLogs(self):
        ...
        _logger.info("removed all call log of pos")
        self.env['pos.cache.database'].search([]).unlink()
        _logger.info("removed all backend change of pos")
        return True
