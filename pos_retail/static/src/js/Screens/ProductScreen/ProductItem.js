odoo.define('pos_retail.ProductItem', function (require) {
    ...
    const RetailProductItem = (ProductItem) => class extends ProductItem {
        constructor() {
            super(...arguments);
            this.state = useState({
                refresh: 'waiting',
        ...
        async _autoSyncBackend() {
            ...
            if (products.length == 1) {
                console.warn('>>> _autoSyncBackend product ID: ' + this.props.product.id)
        ...
        get price() {
            ...
            const formattedUnitPrice = this.env.pos.format_currency(price, 'Product Price');
        ...
        async editProduct() {
            let {confirmed, payload: results} = await this.showPopup('PopUpCreateProduct', {
                title: this.env._t('Edit ') + this.props.product.display_name, product: this.props.product
            })
            if (confirmed && results) {
                ...
                await this.rpc({
                    model: 'product.product', method: 'write', args: [[this.props.product.id], value]
                })
        ...
        async archiveProduct() {
            ...
            if (confirmed) {
                await this.rpc({
                    model: 'product.product', method: 'write', args: [[this.props.product.id], {
        ...
        async addBarcode() {
            let newBarcode = await this.rpc({
                model: 'product.product', method: 'add_barcode', args: [[this.props.product.id]]
            })
            if (newBarcode) {
        ...
        async printBarcode() {
            await this.env.pos.do_action('pos_retail.report_product_product_barcode', {
                additional_context: {
                    active_id: this.props.product.id, active_ids: [this.props.product.id],
                }
            }, {
